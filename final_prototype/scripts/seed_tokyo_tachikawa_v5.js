...
const AREAS = [
  ...
  // Western expansion: extend the same life-circle model toward Tachikawa
  a("kichijoji", "吉祥寺", 35.7031, 139.5797, "S", "shopping", "suburban_commercial"),
  a("tachikawa", "立川", 35.6984, 139.4137, "S", "station", "west_hub"),
  a("mitaka", "三鷹", 35.7027, 139.5600, "A", "residential", "suburban_residential"),
  a("musashisakai", "武蔵境", 35.7020, 139.5447, "A", "residential", "suburban_residential"),
  a("chofu", "調布", 35.6517, 139.5448, "A", "station", "suburban_commercial"),
  a("fuchu", "府中", 35.6722, 139.4800, "A", "shopping", "suburban_commercial"),
  a("kokubunji", "国分寺", 35.7001, 139.4808, "A", "station", "suburban_residential"),
  a("kunitachi", "国立", 35.6849, 139.4677, "A", "residential", "campus_green"),
  a("musashikoganei", "武蔵小金井", 35.7032, 139.5048, "A", "residential", "park_residential"),
  a("higashikoganei", "東小金井", 35.7024, 139.5185, "B", "residential", "campus_green"),
  a("kodaira", "小平", 35.7286, 139.4769, "B", "residential", "park_residential"),
  a("tanashi", "田無", 35.7270, 139.5396, "B", "shopping", "suburban_commercial"),
  a("hoya", "保谷", 35.7488, 139.5688, "B", "residential", "suburban_residential"),
  a("higashikurume", "東久留米", 35.7601, 139.5338, "B", "residential", "park_residential"),
  a("kiyose", "清瀬", 35.7720, 139.5196, "C", "residential", "suburban_residential"),
  a("higashimurayama", "東村山", 35.7600, 139.4658, "B", "residential", "suburban_residential"),
  a("kokubunji_west_green", "国分寺西側緑地", 35.7045, 139.4550, "C", "park", "campus_green"),
  a("nogawa_koganei", "野川小金井", 35.6903, 139.5008, "C", "park", "park_residential"),
  a("koganei_park", "小金井公園", 35.7213, 139.5057, "C", "park", "campus_green"),
  a("tachikawa_north", "立川北側住宅地", 35.7140, 139.4180, "B", "residential", "suburban_residential"),
  a("tachikawa_south", "立川南側商住地", 35.6900, 139.4140, "B", "shopping", "suburban_commercial"),
  a("showa_kinen", "昭和記念公園", 35.7048, 139.3940, "C", "park", "campus_green"),
  a("haijima_edge", "拝島手前外縁", 35.7217, 139.3439, "D", "residential", "outer_sparse"),
  a("tamagawa_tachikawa", "多摩川立川縁", 35.6820, 139.4020, "D", "water", "outer_sparse"),
  a("fussa_edge_gap", "福生手前空白縁", 35.7400, 139.3300, "E", "residential", "outer_sparse")
];
...
const PROFILE_TYPE_WEIGHTS = {
  hyper_core: { station: 1.20, corridor: 1.10, shopping: 1.15, office: 1.10, residential: 0.70, park: 0.72, water: 0.80, night: 1.12, shrine: 0.55 },
  office_core: { station: 1.00, corridor: 1.05, shopping: 0.80, office: 1.55, residential: 0.58, park: 0.62, water: 0.86, night: 0.55, shrine: 0.45 },
  night_core: { station: 1.08, corridor: 1.05, shopping: 1.15, office: 0.60, residential: 0.65, park: 0.58, water: 0.72, night: 1.55, shrine: 0.42 },
  water_bay: { station: 0.80, corridor: 0.90, shopping: 0.92, office: 0.80, residential: 0.72, park: 1.18, water: 1.65, night: 0.55, shrine: 0.50 },
  suburban_commercial: { station: 1.00, corridor: 1.00, shopping: 1.22, office: 0.70, residential: 1.08, park: 0.95, water: 0.82, night: 0.74, shrine: 0.78 },
  suburban_residential: { station: 0.82, corridor: 0.95, shopping: 0.82, office: 0.48, residential: 1.52, park: 1.05, water: 0.88, night: 0.35, shrine: 0.82 },
  campus_green: { station: 0.70, corridor: 0.86, shopping: 0.58, office: 0.40, residential: 1.05, park: 1.58, water: 1.00, night: 0.24, shrine: 1.06 },
  park_residential: { station: 0.72, corridor: 0.90, shopping: 0.70, office: 0.40, residential: 1.38, park: 1.45, water: 1.04, night: 0.28, shrine: 0.92 },
  west_hub: { station: 1.20, corridor: 1.10, shopping: 1.20, office: 0.82, residential: 0.95, park: 1.05, water: 0.82, night: 0.88, shrine: 0.62 },
  outer_sparse: { station: 0.40, corridor: 0.52, shopping: 0.30, office: 0.22, residential: 1.18, park: 1.22, water: 1.25, night: 0.10, shrine: 0.72 }
};

function inferProfile(area) {
  if (area.profile) return area.profile;
  if (area.level === "S" && area.bias === "office") return "office_core";
  if (area.level === "S" && area.bias === "night") return "night_core";
  if (area.bias === "water") return "water_bay";
  if (area.bias === "park") return "park_residential";
  if (area.bias === "residential") return area.level === "D" || area.level === "E" ? "outer_sparse" : "suburban_residential";
  if (area.bias === "shopping") return area.level === "S" ? "hyper_core" : "suburban_commercial";
  return area.level === "S" ? "hyper_core" : "suburban_commercial";
}

function typeWeight(area, type) {
  const profile = PROFILE_TYPE_WEIGHTS[inferProfile(area)] || PROFILE_TYPE_WEIGHTS.suburban_residential;
  return profile[type] || 1;
}

function radiusWeight(area, type) {
  const profile = inferProfile(area);
  if (profile === "outer_sparse") return type === "residential" || type === "park" || type === "water" ? 1.16 : 0.74;
  if (profile === "office_core") return type === "office" || type === "station" ? 0.90 : 0.78;
  if (profile === "night_core") return type === "night" || type === "shopping" ? 0.88 : 0.82;
  if (profile === "water_bay") return type === "water" || type === "park" ? 1.12 : 0.86;
  if (profile === "campus_green" || profile === "park_residential") return type === "park" || type === "residential" ? 1.10 : 0.86;
  return 1;
}

function a(id, label, lat, lng, level, bias, profile = null) {
  return { id, label, center: { lat, lng }, level, bias, profile, records: LEVEL_COUNTS[level] };
}
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (arg === "--records") options.records = Number(argv[++i]);
      ...
      console.log("Usage: node scripts/seed_tokyo_tachikawa_v5.js [--records 150000] [--batch-size 1000] [--dry-run] [--replace-world]");
      process.exit(0);
    }
  }
...
function buildAreaStructure(area) {
  ...
  const zones = TEMPLATE_ZONES.map(([id, suffix, typeRaw, dx, dy, radius, weight]) => {
    ...
    return {
      id: `tokyo_v5_${area.id}_${id}`,
      areaId: area.id,
      label: `${area.label}${suffix}`,
      type,
      center,
      radius: Math.round(radius * scale * radiusWeight(area, type)),
      weight: weight * (type === area.bias ? 1.35 : 1) * typeWeight(area, type),
      ...scene
    };
  });
  const corridors = TEMPLATE_CORRIDORS.map(([id, suffix, typeRaw, points, width]) => {
    ...
    return {
      ...
      width: Math.round(width * scale * radiusWeight(area, type)),
      weight: (type === area.bias ? 1.25 : 1) * typeWeight(area, type),
      ...scene
    };
  });
  const voids = TEMPLATE_VOIDS.map(([id, dx, dy, radius]) => ({
    ...
    radius: Math.round(radius * scale * (inferProfile(area) === "outer_sparse" ? 1.35 : inferProfile(area).includes("green") ? 1.18 : 1))
  }));
...
function buildRecords(targetCount, structures) {
  ...
  for (const area of areaAllocations) {
    ...
    for (let local = 0; local < area.target; local++) {
      ...
      rows.push({
        id: `tokyo-v5-${String(index).padStart(6, "0")}`,
        user_id: `tokyo-v5-user-${String(index % 8200).padStart(5, "0")}`,
        device_id: null,
        record_type: "world",
...
async function main() {
  ...
  if (options.replaceWorld) {
    ...
    for (const prefix of ["central", "asakusa_v3", "tokyo_v4", "tokyo_v5"]) {

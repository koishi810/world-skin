(() => {
      ...
      const ZOOM_ORDER = ["region", "area", "neighborhood", "street", "detail"];
      ...
      const AXIS_VISUAL_MAP = {
        ...
        tension: {
          primaryChannels: ["microAlphaVariance", "microHeightVariance"],
          forbidPrimary:   ["boundaryBias", "edgeHardnessBias"],
          bias: {
            microAlphaVariance:  { min: +0.05, max: +0.30 },
            microHeightVariance: { min: +0.05, max: +0.34 },
          },
        },
        flow: {
          primaryChannels: ["heightLineEndpointOffset", "flowCoherence", "timePersistence"],
          forbidPrimary:   [],
          bias: {
            fluxBias:           { min: -0.12, max: +0.12 },
            endpointOffsetScale:{ min:  0.00, max: +0.35 },
            persistenceBias:    { min:  0.00, max: +0.55 },
          },
        },
      };
      ...
      // finalColor is synthesized from the existing field state. Color remains
      // a restrained gray-skin result, not an independent perceptual axis.
      const COLOR_SYNTHESIS_MAP = {
        inputs: ["strength", "spaciousness", "gravity", "tension", "soundVector"],
        anchors: ["coldBlueGray", "cyanGray", "silverGray", "neutralGray", "warmGray", "amberGray", "rustOrangeGray", "darkPurpleGray", "darkRedBrown", "blueBlack"],
        role: "secondaryAtmosphere",
      };

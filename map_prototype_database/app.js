(() => {
  ...
  const supabase = (() => {
    ...
    if (!url || !key || !window.supabase) {
      console.warn("[MapPrototypeDB] Supabase is not configured");
      return null;
    }
  ...
  function deserializeRecord(row) {
    ...
    const word = row.word || (Array.isArray(row.selected_words) ? row.selected_words[0] : null);
    return {
      id: row.id,
      userId: row.user_id,
      ...
      hour: Number.isFinite(Number(row.hour)) ? Number(row.hour) : null,
      weekday: row.weekday,
      noiseLevel: row.noise_level ?? row.noise,
      turbulence: row.turbulence ?? row.flux,
      peak: row.peak,
      mobility: row.mobility || row.movement,
      direction: row.direction,
      duration: row.duration,
      word,
      selectedWords: Array.isArray(row.selected_words) && row.selected_words.length
        ? row.selected_words
        : (word ? [word] : []),
      ...
      zoneId: row.zone_id,
      source: row.source,
      noise: row.noise ?? row.noise_level,
      flux: row.flux ?? row.turbulence,
      movement: row.movement || row.mobility,
  ...
  async function loadSupabaseWorldRows({ bounds = null, limit = remoteWorldLimitForZoom() } = {}) {
    ...
    for (let from = 0; from < limit; from += pageSize) {
      ...
      if (error) {
        console.warn("[MapPrototypeDB] load world records failed", error);
        return rows;
      }
  ...
  function applyRemoteWorldRecords(rows) {
    WORLD_RECORDS = rows.map(deserializeRecord).filter(Boolean);
  ...
  async function refreshViewportWorldRecords() {
    ...
    if (requestId !== state.remoteRecordsRequestId) return;
    applyRemoteWorldRecords(rows);
  }
  ...
  function scheduleViewportWorldRefresh(delay = 360) {
    ...
    state.remoteRecordsTimer = setTimeout(() => {
      refreshViewportWorldRecords().catch(error => {
        console.warn("[MapPrototypeDB] viewport record refresh failed", error);
      });
    }, delay);
  }
  ...
  function initRealMap() {
    ...
    state.map.on("zoomend", () => { applyMapVisualMode(); startFieldTransition(); scheduleViewportWorldRefresh(160); });
    ["moveend", "rotateend", "pitchend"].forEach(e => state.map.on(e, () => {
      invalidateField(false);
      scheduleViewportWorldRefresh(220);
    }));
  ...
  function initLocation() {
    ...
    navigator.geolocation.watchPosition(
      pos => {
        state.position = { lat: pos.coords.latitude, lng: pos.coords.longitude };
        state.heading  = Number.isFinite(pos.coords.heading) ? pos.coords.heading : state.heading;
        scheduleViewportWorldRefresh(600);
      },
      () => {},
